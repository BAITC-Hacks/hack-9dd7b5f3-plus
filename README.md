...
5. **Watch it think in real time:** *Debug* page, or in a terminal: `curl -N http://localhost:8080/api/debug/events` — every step (triage, retrieval candidates, prompt, model tokens, decision, actions, TTS) is an event. `GET /api/prompt` shows the exact system prompt. `node scripts/ws_smoke.mjs tests/audio/02_ru_topic_switch.wav` streams a recording through the voice WebSocket exactly like the browser does and prints the events with timings (`--text "…"` for a text turn, `--save reply.pcm` to keep the audio).
