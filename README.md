...
>
> Deployed link: `<fill in if deployed>` · Demo access: none needed (keyless mode works out of the box)
...
5. **Watch it think in real time:** *Debug* page, or in a terminal: `curl -N http://localhost:8080/api/debug/events` — every step (triage, retrieval candidates, prompt, model tokens, decision, actions, TTS) is an event. `GET /api/prompt` shows the exact system prompt.
