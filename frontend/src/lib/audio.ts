...
export async function playPCM(
  ...
  signal: AbortSignal,
  onStart: (firstByteMS: number, playbackAt: number) => void,
) {
  await context.resume();
  const res = await fetch(
    `${API}/api/sessions/${sessionID}/turns/${turnID}/speech`,
    { signal },
  );
  if (!res.ok) {
    const err = await res.json();
    throw new Error(err.error || "Ошибка синтеза речи");
  }
  if (!res.body) throw new Error("Нет аудиопотока");
  const reader = res.body.getReader();
  let spare: number | undefined;
  let next = context.currentTime + 0.03;
  let first = true;
  ...
  try {
    for (;;) {
      ...
      const bytes = new Uint8Array(
        value.length + (spare === undefined ? 0 : 1),
      );
      let offset = 0;
      if (spare !== undefined) {
        bytes[0] = spare;
      ...
      next = Math.max(next, context.currentTime + 0.015);
      node.start(next);
      nodes.push(node);
      if (first) {
        first = false;
        ...
        onStart(
          Number(res.headers.get("X-TTS-First-Byte-MS") || 0),
          performance.now() + (next - context.currentTime + latency) * 1000,
        );
      }
...
export function browserRecognition(
  ...
  onFinal: (text: string, endAt: number, sttMS: number) => void,
  ...
  if (!Constructor)
    throw new Error(
      "Браузер не поддерживает SpeechRecognition. Подключите OpenAI или используйте текст / аудиофайл.",
    );
  ...
  rec.onerror = (e) => {
    if (!canceled)
      onError(`Распознавание: ${e.error}. Проверьте разрешение микрофона.`);
  };
  ...
  rec.onend = () => {
    if (!canceled) {
      if (final.trim())
        onFinal(final.trim(), Number.NaN, performance.now() - lastPartial);
...
export async function realtimeCapture(
  ...
  function commit() {
    ...
    if (!speaking) {
      fail(
        "Не удалось обнаружить речь. Попробуйте говорить ближе к микрофону.",
      );
      return;
    }
    committed = true;
    ...
    onState("Распознавание");
    clearTimeout(deadline);
    deadline = setTimeout(
      () => fail("Истекло время ожидания транскрипта"),
      20000,
    );
  }
  dc.onmessage = (message) => {
    try {
      ...
      if (event.type === "error") {
        fail(event.error?.message || "Ошибка Realtime API");
        return;
      }
      if (event.type === "conversation.item.input_audio_transcription.failed") {
        fail("Потоковое распознавание не удалось");
        return;
      }
      ...
      if (
        ...
        const latency = performance.now() - endAt;
        close();
        if (text) onFinal(text, endAt, latency);
        else onError("Пустой транскрипт");
      }
    } catch {
      fail("Некорректное событие распознавания");
    }
  };
  pc.onconnectionstatechange = () => {
    if (!closed && ["failed", "disconnected"].includes(pc.connectionState))
      fail("Соединение с распознаванием прервано");
  };
  try {
    const offer = await pc.createOffer();
    await pc.setLocalDescription(offer);
    const res = await fetch(`${API}/api/realtime/connect`, {
      method: "POST",
    ...
    if (!res.ok) {
      ...
      throw new Error(e.error || "Realtime недоступен");
    }
    ...
    onState("Слушаю");
    deadline = setTimeout(() => fail("Лимит записи 30 секунд"), 30000);
    ...
    timer = setInterval(() => {
      ...
      if (rms > 0.018) {
        ...
        if (voicedFrames >= 3) speaking = true;
      } else if (!speaking) {
        voicedFrames = 0;
      }
