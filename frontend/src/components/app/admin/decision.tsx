...
export function DecisionCard({ view }: { view: TurnView }) {
  ...
  return (
    <Section
      title="Что понял робот"
      hint={view.transcript ? `«${view.transcript}»` : undefined}
      right={sc?.priority === "urgent" ? <Badge variant="error" size="sm">срочно</Badge> : d?.is_continuation ? <Badge variant="info" size="sm">продолжение</Badge> : undefined}
    >
      {!top && !routing && <div className="py-6 text-center text-sm text-muted-foreground">Скажите или напишите что-нибудь слева.</div>}
      {routing && <div className="flex items-center gap-2 py-2 text-sm text-muted-foreground"><span className="size-1.5 animate-pulse rounded-full bg-brand" /> выбираю сценарий…</div>}

      {top && (
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="text-xs text-muted-foreground">Сценарий</div>
            <div className="mt-0.5 text-lg font-medium leading-snug">{scenarioLabel(top.scenario_id, "ru")}</div>
      ...
      {top && (
        <div className="mt-3 rounded-lg bg-muted px-3 py-2 text-sm leading-relaxed">
          <span className="text-muted-foreground">Почему: </span>{d?.reason || top.reason || <Dash />}
        </div>
      )}

      {d && d.scenarios.length > 1 && (
        <div className="mt-3 text-sm">
          <div className="text-xs text-muted-foreground">В этой реплике несколько просьб, по порядку:</div>
          <ol className="mt-1 space-y-0.5">
            {d.scenarios.map((s, i) => (
              <li key={s.scenario_id} className="flex justify-between gap-3">
                <span>{i + 1}. {scenarioLabel(s.scenario_id, "ru")} <span className="font-mono text-xs text-muted-foreground">{s.scenario_id}</span></span>
      ...
      {cands.length > 0 && (
        <div className="mt-4">
          <div className="mb-1.5 flex items-center justify-between text-xs text-muted-foreground">
            <span>Кандидаты{view.source === "live" && view.stage === "router" ? " — обновляются" : ""}</span>
            ...
            {cands.map((c) => {
              ...
              return (
                <li key={c.scenario_id} className="grid grid-cols-[minmax(0,1fr)_120px_44px] items-center gap-3 text-sm">
                  <span className={cn("truncate", !chosen && "text-muted-foreground")}>{scenarioLabel(c.scenario_id, "ru")} <span className="font-mono text-xs text-muted-foreground/70">{c.scenario_id}</span></span>
      ...
      {sc && sc.not_this_if.length > 0 && (
        <details className="mt-4 text-sm">
          <summary className="cursor-pointer text-xs text-muted-foreground hover:text-foreground">Правила разграничения из каталога ({sc.not_this_if.length})</summary>
          <ul className="mt-2 space-y-1.5">
            {sc.not_this_if.map((r, i) => (
              <li key={i} className="rounded-lg bg-muted px-3 py-2 text-xs leading-relaxed">
                <span className="text-muted-foreground">Не этот сценарий, если: </span>{r.condition}
                <span className="text-muted-foreground"> → </span><span className="font-mono">{r.use_instead}</span> {scenarioLabel(r.use_instead, "ru")}
              </li>
            ))}
          </ul>
        </details>
