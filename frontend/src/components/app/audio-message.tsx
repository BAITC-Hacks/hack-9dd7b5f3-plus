...
function Player({ saved, turn }: { saved: SavedAudio; turn: number }) {
  ...
  return <div className="min-w-0">
    <div className="flex items-center gap-3">
      <button type="button" aria-label={playing ? "Приостановить ответ" : "Воспроизвести ответ"}
        ...
        onClick={async () => {
          ...
          else try { await replayAudio(saved); } catch { setError("Не удалось включить звук. Нажмите Play ещё раз."); }
      ...
      <div className="relative h-10 min-w-0 flex-1">
        ...
        <input type="range" min={0} max={duration || 1} step={0.01} value={Math.min(time, duration || 1)} disabled={!duration}
          aria-label="Перемотка аудиоответа" aria-valuetext={`${clock(time)} из ${clock(duration)}`}
      ...
      <a href={saved.url} download={`bagyt-answer-${turn}.mp3`} aria-label="Скачать аудиоответ" className="shrink-0 text-muted-foreground hover:text-foreground"><Download className="size-4" /></a>
    </div>
    <div className="mt-1 flex justify-between pl-[52px] text-[11px] tabular-nums text-muted-foreground"><span>{playing ? "Воспроизводится" : "Аудиоответ"}</span><span>{clock(time)} / {clock(duration)}</span></div>
...
export function AudioMessage({ message }: { message: Message }) {
  return <div className="w-[320px] max-w-full">
    {message.audio ? <Player saved={message.audio} turn={message.turn} /> : <div className="flex items-center gap-2 py-2 text-xs text-muted-foreground"><Volume2 className="size-4" />{message.audioState === "unavailable" ? "Запись недоступна" : "Готовлю аудиоответ…"}</div>}
    <div className="mt-3 border-t border-border/60 pt-3">
      <div className="mb-1 text-[10px] uppercase tracking-wider text-muted-foreground">Транскрипция</div>
