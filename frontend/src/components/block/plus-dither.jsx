...
void main(){
  ...
  if(ca < 0.05) discard;
  vec3 blue = vec3(0.184, 0.416, 0.820);
  vec3 cyan = vec3(0.604, 0.710, 0.929);
  vec3 violet = vec3(0.165, 0.369, 0.733);
 ...
 * PLUS dither — ObsidianUI dither-canvas driven by a static image instead of a video.
 * @param {{ imageSrc?: string, className?: string, style?: import("react").CSSProperties, paused?: boolean }} props
 */
export function PlusDither({
  imageSrc = "/landing/plus.svg",
  className,
  style,
  paused = false,
} = {}) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const draw = () => {
        if (!video.complete || !video.naturalWidth || disposed) return;
        const ts = now();
  ...
  return (
    <div className={cn("relative h-[28rem] w-full overflow-hidden bg-paper", className)} style={style}>
      <div
        role="img"
        aria-label="Плюс из точек — знак команды Plus"
        className="pointer-events-none absolute inset-0"
        style={{
          backgroundImage: "radial-gradient(circle at 60% 45%, #2f6ad1 0%, #5579c6 35%, #bed2ef 70%, transparent 90%)",
