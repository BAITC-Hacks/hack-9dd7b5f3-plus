...
- [ ] 12:30 — grab promo codes (hackathon page / Telegram bot). ChatGPT Pro: confirm **only if "Due today: $0"**, note next billing date. OpenAI API credits → Billing → Promotions. NVIDIA Brev → Billing → Redeem Code. NVIDIA Build API key → build.nvidia.com/settings/api-keys. Details: docs/HACKATHON_RULES.md §12.3.
...
|------|------|---------|---------|-------------------|
| 13:00–13:15 | Read ALL cases of the track → pick **one** case → paste into `docs/CASE.md`; launch Deep Research (`docs/DEEP_RESEARCH_PROMPT.md`) | Go skeleton: `/healthz`, config, DB conn, Dockerfile | Next.js skeleton + Tailwind + shadcn + ObsidianUI registry | |
